...
abstract public class Task implements Writable, Configurable {
  ...
  /**
   * 管理mapper的输出文件
   * 输出路径为{mapred.local.dir}下的对应目录下，该路径可能有多个，按照一定条件选择其中一个
   *
   * 输出的文件包括 spill和file及分别对应的索引文件
   *    file为spill进行merge和排序之后的结果文件
   *  在map中是进行对应文件的创建和处理，最后会在reduce中进行获取
   * */
  ...
  public static String normalizeStatus(String status, Configuration conf) {
    ...
    /**
     * 提交计数器和状态报表，实现了计数报告Reporter和状态报告statusReporter
     * TaskRporter是一个独立的线程，通过TaskUmbilicalProtocol利用RPC向JobTracker报告Task的执行情况
     * */
  @InterfaceAudience.LimitedPrivate({"MapReduce"})
  @InterfaceStability.Unstable
  ...
  /**ted for a
   * An updater that tracks the last number reporgiven file
